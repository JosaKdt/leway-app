...
│   └── recommandation.dart
├── constants/
│   ├── colors.dart               → palette bleu-violet LÉWAY
...
| Élément | Valeur |
|---------|--------|
| Couleur principale (`primary`) | `#2D4EC8` — bleu-violet signature |
| Couleur moyenne (`primaryMedium`) | `#5B7FE8` — icônes, états hover, underlines |
| Couleur claire (`primaryLight`) | `#EEF2FF` — fond des cartes, chips de filières |
| Accent (`accent`) | `#7C3AED` — badges RIASEC, highlights importants |
| Fond (`background`) | `#F7F8FC` — légèrement bleuté, cohérent avec la dominante |
| Dégradé hero (`heroGradient`) | `#2D4EC8` → `#7C3AED` — onboarding, effet "app sérieuse" |
| Style | Moderne, tech, "nouvelle génération" |
| Langues | Français / Anglais (switch in-app) |
| Cible | Android — bacheliers béninois |

> Inspiration visuelle : Notion, Linear, plateformes edtech modernes.
...
|-------|---------|--------|
| Splash + fondations | `splash_screen.dart` | 🔄 En cours (`feature/splash-screen`) |
